...
export class SchemaParser {
    ...
    public parse(file: SourceFile): Schema {
        ...
        if (classes.length > 1) {
            throw new TooMuchSchemaError(path.relative(this.config.schemas, schema.path), file.getClasses());
        }
        schema.cd = classes[0];
        if (!schema.cd) {
            throw new SchemaNotFoundError(path.relative(this.config.schemas, schema.path));
        }
